...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    def->enum_values.push_back("oidc");
    ...
    def->enum_labels.push_back("OIDC (Generic Backend)");
    def->mode = comAdvanced;
    def->cli = ConfigOptionDef::nocli;
    def->set_default_value(new ConfigOptionEnum<PrintHostType>(htOctoPrint));

    def = this->add("oidc_scope", coString);
    def->label = L("OIDC scope");
    def->tooltip = L("Space-separated OAuth2 scope string requested from the OIDC backend for this printer "
                   "(for example \"p1:status p1:upload p1:trigger\"). The backend decides which printer "
                   "this profile maps to based on this scope.");
    def->mode = comAdvanced;
    def->cli = ConfigOptionDef::nocli;
    def->set_default_value(new ConfigOptionString(""));
